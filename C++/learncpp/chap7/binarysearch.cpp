...
int recBinarySearch(int *array, int target, int min, int max)
{
    int mid = (min + max) / 2;
    if (min > max)
        return -1;
    else if (array[mid] < target)
        return recBinarySearch(array, target, mid+1, max);
    else if (array[mid] > target)
        return recBinarySearch(array, target, min, max-1);
    else
        return mid;
}
...
int main()
{
    ...
    {
        // See if our test value is in the array
        int index = recBinarySearch(array, testValues[count], 0, 14);
        // If it matches our expected value, then great!
        if (index == expectedValues[count])
             std::cout << "test value " << testValues[count] << " passed!\n";
        else // otherwise, our binarySearch() function must be broken
